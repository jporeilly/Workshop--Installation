#!/bin/bash
# =============================================================================
# Pentaho Server 11 Docker Deployment Script
# =============================================================================
#
# Automated deployment script for Pentaho Server 11 with MySQL on Ubuntu 24.04
#
# Usage:
#   ./deploy.sh
#
...
#   - Ports 8090, 3306, 8050 available
#
...
#
#   3. Startup Phase
#      - Starts MySQL container and waits for health
#      - Starts Pentaho Server container
#      - Starts Adminer (optional database admin UI)
#
#   4. Verification
...
check_port "${MYSQL_PORT:-3306}" "MySQL" || PORTS_OK=false
check_port "${ADMINER_PORT:-8050}" "Adminer" || PORTS_OK=false
...
# Start Adminer
echo -e "${BLUE}→ Starting Adminer...${NC}"
if docker compose up -d adminer; then
    echo -e "${GREEN}✓ Adminer container started${NC}"
else
    echo -e "${YELLOW}⚠ Failed to start Adminer (optional service)${NC}"
fi
...
echo ""
echo -e "  ${BLUE}Adminer (Database Admin):${NC}"
echo "    URL: http://localhost:${ADMINER_PORT:-8050}"
echo "    Server: repository"
echo "    Login: root / ${MYSQL_ROOT_PASSWORD:-password}"
echo ""
...
echo ""
echo "Helper scripts:"
echo "  Backup database:  ./scripts/backup-mysql.sh"
echo "  Restore database: ./scripts/restore-mysql.sh <backup-file>"
