...
# =============================================================================
#
# Automated deployment script for Pentaho Server 11 with SQL Server on Ubuntu 24.04
#
# Usage:
#   ./deploy.sh
#
...
#
#   3. Startup Phase
#      - Starts SQL Server container and waits for health
...
#   - MSSQL_SA_PASSWORD (default: Password123!)
...
check_port "${MSSQL_PORT:-1433}" "SQL Server" || PORTS_OK=false
...
echo -e "${BLUE}→ Starting SQL Server database...${NC}"
if docker compose up -d mssql; then
    echo -e "${GREEN}✓ SQL Server container started${NC}"
else
    echo -e "${RED}✗ Failed to start SQL Server${NC}"
    exit 1
fi
...
for i in {1..30}; do
    ...
    if [ $i -eq 30 ]; then
        echo -e "${RED}✗ SQL Server failed to become healthy${NC}"
...
echo ""
echo "Helper scripts:"
echo "  Backup database:  ./scripts/backup-mssql.sh"
echo "  Restore database: ./scripts/restore-mssql.sh <backup-file>"
