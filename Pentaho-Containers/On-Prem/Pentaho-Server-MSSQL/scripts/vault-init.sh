...
#   5. Stores database credentials in Vault (matches MSSQL init scripts)
...
#   2. MSSQL container starts → runs db_init_mssql/*.sql scripts
...
#   5. Pentaho connects to MSSQL using Vault credentials ✓
#
#   IMPORTANT: On clean deployment, passwords are NOT rotated automatically.
#   The default password "password" is used to ensure MSSQL and Vault match.
...
# Default passwords - MUST match db_init_mssql/*.sql scripts
# These are the passwords MSSQL logins are created with
...
# Note: The suffix ensures SQL Server password complexity requirements are met
generate_password() {
    local length=${1:-24}
...
# -----------------------------------------------------------------------------
# On CLEAN deployment: Use default passwords (matches MSSQL init scripts)
...
if [ -f "$GENERATED_PASSWORDS_FILE" ]; then
    ...
    # Clean deployment - use default passwords that match MSSQL init scripts
    # This ensures Pentaho can connect immediately after deployment
    echo "    Clean deployment detected"
    echo "    Using default passwords (matches MSSQL init scripts)"
    echo ""
...
# -----------------------------------------------------------------------------
echo ">>> Storing MSSQL credentials in Vault..."
echo "    Path: secret/data/pentaho/mssql"

curl -s --header "X-Vault-Token: $ROOT_TOKEN" \
    --request POST \
    --data "{
        \"data\": {
            \"sa_password\": \"${MSSQL_SA_PASSWORD:-Pentaho@2026}\",
            \"jcr_user\": \"jcr_user\",
            ...
            \"jdbc_url\": \"jdbc:sqlserver://repository:1433;databaseName=pentaho;encrypt=true;trustServerCertificate=true\",
            \"passwords_source\": \"$PASSWORDS_SOURCE\",
            \"updated_at\": \"$(date -Iseconds)\"
        }
    }" \
    "${VAULT_ADDR}/v1/secret/data/pentaho/mssql"

# Verify secrets were stored
echo "    Verifying secrets storage..."
VERIFY_RESULT=$(curl -s --header "X-Vault-Token: $ROOT_TOKEN" \
    "${VAULT_ADDR}/v1/secret/data/pentaho/mssql" | jq -r '.data.data.pentaho_user // "FAILED"')
