...
#   5. Stores database credentials in Vault (matches PostgreSQL init scripts)
...
#   2. PostgreSQL container starts → runs db_init_postgres/*.sql scripts
...
#   5. Pentaho connects to PostgreSQL using Vault credentials ✓
#
#   IMPORTANT: On clean deployment, passwords are NOT rotated automatically.
#   The default password "password" is used to ensure PostgreSQL and Vault match.
...
# Default passwords - MUST match db_init_postgres/*.sql scripts
# These are the passwords PostgreSQL users are created with
...
# -----------------------------------------------------------------------------
# On CLEAN deployment: Use default passwords (matches PostgreSQL init scripts)
...
if [ -f "$GENERATED_PASSWORDS_FILE" ]; then
    ...
    # Clean deployment - use default passwords that match PostgreSQL init scripts
    # This ensures Pentaho can connect immediately after deployment
    echo "    Clean deployment detected"
    echo "    Using default passwords (matches PostgreSQL init scripts)"
    echo ""
...
# -----------------------------------------------------------------------------
echo ">>> Storing PostgreSQL credentials in Vault..."
echo "    Path: secret/data/pentaho/postgresql"
...
curl -s --header "X-Vault-Token: $ROOT_TOKEN" \
    ...
    --data "{
        \"data\": {
            ...
            \"jdbc_url\": \"jdbc:postgresql://repository:5432/pentaho\",
            \"passwords_source\": \"$PASSWORDS_SOURCE\",
            \"updated_at\": \"$(date -Iseconds)\"
        }
    }" \
    "${VAULT_ADDR}/v1/secret/data/pentaho/postgresql"

# Verify secrets were stored
echo "    Verifying secrets storage..."
VERIFY_RESULT=$(curl -s --header "X-Vault-Token: $ROOT_TOKEN" \
    "${VAULT_ADDR}/v1/secret/data/pentaho/postgresql" | jq -r '.data.data.pentaho_user // "FAILED"')
