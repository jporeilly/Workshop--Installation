...
#
# WHAT IT DOES:
#   1. Validates prerequisites (Oracle running, Vault unsealed)
#   2. Generates cryptographically secure passwords
#   3. Updates Oracle users with new passwords
...
#   │                    ↓                                             │
#   │  2. Update Oracle users (ALTER USER)                             │
#   │                    ↓                                             │
...
#   1. Check which component failed (Oracle or Vault)
...
#      - Check Vault secrets match Oracle passwords
#      - Restart Pentaho: docker compose restart pentaho-server
#
# FILES MODIFIED:
#   /vault/data/generated-passwords.json  - Updated with new passwords
#   Vault: secret/data/pentaho/oracle     - Updated with new passwords
#   Oracle: users for all accounts        - Passwords updated
#
# =============================================================================
...
VAULT_SECRET_PATH="secret/data/pentaho/oracle"
...
# Mask password for display (shows first 4 and last 4 characters)
mask_password() {
    local pass="$1"
...
check_prerequisites() {
    ...
    # Check Oracle container is responding
    if ! docker exec pentaho-oracle sqlplus -s sys/$ORACLE_PASSWORD@localhost/$ORACLE_SID as sysdba <<< "SELECT 1 FROM DUAL;" > /dev/null 2>&1; then
        echo -e "${RED}ERROR: Oracle container is not responding${NC}"
        exit 1
    fi
    echo -e "${GREEN}✓ Oracle is running${NC}"
    ...
    # Check for vault-keys.json (needed to get root token)
...
# -----------------------------------------------------------------------------
# Get Vault root token from keys file
# -----------------------------------------------------------------------------
get_vault_token() {
...
# -----------------------------------------------------------------------------
# Update Oracle user password
# -----------------------------------------------------------------------------
update_oracle_password() {
    local user="$1"
    local new_password="$2"

    echo "  Updating Oracle password for $user..."
    ...
    docker exec pentaho-oracle sqlplus -s sys/$ORACLE_PASSWORD@localhost/$ORACLE_SID as sysdba <<EOF > /dev/null 2>&1
ALTER USER $user IDENTIFIED BY "$new_password";
EXIT;
EOF

    if [ $? -eq 0 ]; then
        echo -e "  ${GREEN}✓ Oracle password updated for $user${NC}"
        return 0
    else
        echo -e "  ${RED}✗ Failed to update Oracle password for $user${NC}"
        return 1
    fi
}
...
update_vault_secret() {
    ...
    # Get existing secrets to preserve other fields (like oracle_password, jdbc_url)
    EXISTING=$(curl -s --header "X-Vault-Token: $token" \
        "http://localhost:$VAULT_PORT/v1/$VAULT_SECRET_PATH" 2>/dev/null)

    ORACLE_PASS=$(echo "$EXISTING" | jq -r '.data.data.oracle_password // "'"$ORACLE_PASSWORD"'"')
    JDBC_URL=$(echo "$EXISTING" | jq -r '.data.data.jdbc_url // "jdbc:oracle:thin:@repository:1521/FREEPDB1"')
...
# -----------------------------------------------------------------------------
# Update generated-passwords.json
# This file is read by vault-init.sh on subsequent container restarts
# to maintain password consistency
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Restart Pentaho to apply new credentials
# -----------------------------------------------------------------------------
restart_pentaho() {
...
main() {
    echo "============================================"
    echo " Pentaho Secret Rotation (Oracle)"
    echo "============================================"
    ...
    if [ "$DRY_RUN" = false ]; then
        echo -e "${YELLOW}This will update passwords in Oracle and Vault.${NC}"
    ...
    echo ""
    echo -e "${BLUE}Updating Oracle passwords...${NC}"

    # Update Oracle passwords
    ORACLE_FAILED=false
    if [ -z "$SPECIFIC_USER" ] || [ "$SPECIFIC_USER" = "jcr_user" ]; then
        update_oracle_password "jcr_user" "$NEW_JCR" || ORACLE_FAILED=true
    fi
    if [ -z "$SPECIFIC_USER" ] || [ "$SPECIFIC_USER" = "pentaho_user" ]; then
        update_oracle_password "pentaho_user" "$NEW_PENTAHO" || ORACLE_FAILED=true
    fi
    if [ -z "$SPECIFIC_USER" ] || [ "$SPECIFIC_USER" = "hibuser" ]; then
        update_oracle_password "hibuser" "$NEW_HIBUSER" || ORACLE_FAILED=true
    fi

    if [ "$ORACLE_FAILED" = true ]; then
        echo -e "${RED}ERROR: Oracle password update failed. Aborting to prevent inconsistency.${NC}"
        exit 1
    fi
    ...
    if [ $? -ne 0 ]; then
        echo -e "${RED}ERROR: Vault update failed. Oracle has new passwords but Vault has old ones!${NC}"
        echo "You may need to manually update Vault or revert Oracle passwords."
        exit 1
    fi

    echo ""

    # Update passwords file for vault-init.sh consistency
