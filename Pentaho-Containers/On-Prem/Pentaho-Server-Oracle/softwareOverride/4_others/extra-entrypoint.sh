...
#   │                    ↓                                              │
#   │  5. Fetch credentials from secret/data/pentaho/oracle             │
#   │                    ↓                                              │
...
#   VAULT_SECRET_PATH   - Path to secrets (default: secret/data/pentaho/oracle)
...
VAULT_SECRET_PATH="${VAULT_SECRET_PATH:-secret/data/pentaho/oracle}"
...
# Since jq is not available in the Pentaho container, we use grep/sed
# -----------------------------------------------------------------------------
...
# AppRole is a machine-oriented auth method (role_id + secret_id = token)
# Returns: client_token on stdout (status messages go to stderr)
# -----------------------------------------------------------------------------
authenticate_vault() {
...
# Reads from secret/data/pentaho/oracle path
...
# Replaces password attributes for hibuser, pentaho_user, and jcr_user
# Creates a backup before modification (restored on failure)
# -----------------------------------------------------------------------------
update_context_xml() {
...
main() {
    ...
    # Update context.xml with fetched credentials
