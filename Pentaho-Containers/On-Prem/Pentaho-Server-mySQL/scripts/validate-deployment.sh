...
#   4. MySQL is responding and databases exist
...
if docker compose ps 2>/dev/null | grep -q "pentaho-mysql.*Up"; then
        ...
        if [ -n "$ROLE_ID" ] && [ -n "$SECRET_ID" ]; then
            ...
            if [ -n "$CLIENT_TOKEN" ]; then
                ...
                SECRETS_CHECK=$(curl -s --header "X-Vault-Token: $CLIENT_TOKEN" \
                    "http://localhost:$VAULT_PORT/v1/secret/data/pentaho/mysql" 2>/dev/null || echo "{}")
                ...
                if [ -n "$PENTAHO_USER" ]; then
                    ...
                    # Display secrets
                    echo ""
                    echo "  Secrets stored in Vault (secret/pentaho/mysql):"
                    echo "  ┌──────────────────────────────────────────────────────"
...
# =============================================================================
# Check 4: MySQL Health
# =============================================================================
echo -e "${BLUE}→ Checking MySQL health...${NC}"
...
JACKRABBIT_TABLES=$(docker exec pentaho-mysql mysql -uroot -p"$MYSQL_PASSWORD" jackrabbit -e "SHOW TABLES;" 2>/dev/null | wc -l || echo "0")
...
QUARTZ_TABLES=$(docker exec pentaho-mysql mysql -uroot -p"$MYSQL_PASSWORD" quartz -e "SHOW TABLES LIKE 'QRTZ6_%';" 2>/dev/null | wc -l || echo "0")
...
VOLUMES=$(docker volume ls 2>/dev/null | grep -E "pentaho.*mysql_data|pentaho.*solutions|pentaho.*data|vault_data" || true)

if echo "$VOLUMES" | grep -q "mysql_data"; then
