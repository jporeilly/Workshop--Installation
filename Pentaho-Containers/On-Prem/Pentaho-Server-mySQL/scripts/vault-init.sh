...
#   5. Stores database credentials in Vault (matches MySQL init scripts)
...
#   2. MySQL container starts → runs db_init_mysql/*.sql scripts
...
#   5. Pentaho connects to MySQL using Vault credentials ✓
#
#   IMPORTANT: On clean deployment, passwords are NOT rotated automatically.
#   The default password "password" is used to ensure MySQL and Vault match.
...
# Default passwords - MUST match db_init_mysql/*.sql scripts
# These are the passwords MySQL users are created with
...
# -----------------------------------------------------------------------------
# On CLEAN deployment: Use default passwords (matches MySQL init scripts)
...
if [ -f "$GENERATED_PASSWORDS_FILE" ]; then
    ...
    # Clean deployment - use default passwords that match MySQL init scripts
    # This ensures Pentaho can connect immediately after deployment
    echo "    Clean deployment detected"
    echo "    Using default passwords (matches MySQL init scripts)"
    echo ""
...
# -----------------------------------------------------------------------------
echo ">>> Storing MySQL credentials in Vault..."
echo "    Path: secret/data/pentaho/mysql"
...
curl -s --header "X-Vault-Token: $ROOT_TOKEN" \
    ...
    --data "{
        \"data\": {
            ...
            \"jdbc_url\": \"jdbc:mysql://repository:3306/pentaho\",
            \"passwords_source\": \"$PASSWORDS_SOURCE\",
            \"updated_at\": \"$(date -Iseconds)\"
        }
    }" \
    "${VAULT_ADDR}/v1/secret/data/pentaho/mysql"

# Verify secrets were stored
echo "    Verifying secrets storage..."
VERIFY_RESULT=$(curl -s --header "X-Vault-Token: $ROOT_TOKEN" \
    "${VAULT_ADDR}/v1/secret/data/pentaho/mysql" | jq -r '.data.data.pentaho_user // "FAILED"')
