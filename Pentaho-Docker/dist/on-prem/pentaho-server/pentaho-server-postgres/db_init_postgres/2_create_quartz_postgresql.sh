...
# Create Quartz scheduler database and user
# Uses environment variables: QUARTZ_DB_USER, QUARTZ_DB_PASSWORD

# Create user and database
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "postgres" <<-EOSQL
    DROP USER IF EXISTS ${QUARTZ_DB_USER};
    CREATE ROLE ${QUARTZ_DB_USER} LOGIN PASSWORD '${QUARTZ_DB_PASSWORD}';
    
    SELECT 'CREATE DATABASE quartz WITH OWNER = ${QUARTZ_DB_USER} ENCODING = ''UTF8'' TABLESPACE = pg_default'
    WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'quartz')\gexec
    
    GRANT ALL ON DATABASE quartz TO ${QUARTZ_DB_USER};
EOSQL

# Create Quartz tables
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "quartz" <<-EOSQL
...
ALTER TABLE qrtz6_fired_triggers OWNER TO ${QUARTZ_DB_USER};
ALTER TABLE qrtz6_paused_trigger_grps OWNER TO ${QUARTZ_DB_USER};
ALTER TABLE qrtz6_scheduler_state OWNER TO ${QUARTZ_DB_USER};
ALTER TABLE qrtz6_locks OWNER TO ${QUARTZ_DB_USER};
ALTER TABLE qrtz6_simple_triggers OWNER TO ${QUARTZ_DB_USER};
ALTER TABLE qrtz6_cron_triggers OWNER TO ${QUARTZ_DB_USER};
ALTER TABLE qrtz6_blob_triggers OWNER TO ${QUARTZ_DB_USER};
ALTER TABLE qrtz6_simprop_triggers OWNER TO ${QUARTZ_DB_USER};
ALTER TABLE qrtz6_triggers OWNER TO ${QUARTZ_DB_USER};
ALTER TABLE qrtz6_job_details OWNER TO ${QUARTZ_DB_USER};
ALTER TABLE qrtz6_calendars OWNER TO ${QUARTZ_DB_USER};

EOSQL

echo "Quartz database 'quartz' created with user '${QUARTZ_DB_USER}'"
